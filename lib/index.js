...
function resolve_typed_value(nextql, value, valueModel, query, context, info) {
	return Promise.all(Object.keys(query).map(path => {
		...
		return Promise.resolve(valueModel.get(value, fieldName, query[path].$params, context)).then(fieldValue => {
			const newInfo = {
				result: info.result,
				path: info.path.concat(path)
			};
			...
			if (valueModel.computed[fieldName] || valueModel.fields[fieldName] === 1) {
				return resolve_value(nextql, fieldValue, 1, query[path], context, newInfo);
			}

			if (typeof valueModel.fields[fieldName] == "string") {
				return resolve_value(nextql, fieldValue, nextql.model(valueModel.fields[fieldName]), query[path], context, newInfo);
			}

			if (isPlainObject(valueModel.fields[fieldName])) {
				return resolve_value(nextql, fieldValue, new AnonymousModel(valueModel.fields[fieldName]), query[path], context, newInfo);
			}
		});
	}));
}
...
function resolve_value(nextql, value, valueModel, query, context, info) {
	...
	if (typeof value.then == "function") {
		return value.then(v => resolve_value(nextql, v, valueModel, query, context, info));
	}

	if (Array.isArray(value)) {
		return Promise.all(value.map((v, idx) => resolve_value(nextql, v, valueModel, query, context, {
			result: info.result,
...
function execute_method(nextql, model, methodName, query, context, info) {
	...
	return Promise.resolve(model.call(methodName, params, context)).then(value => resolve_value(nextql, value, 1, query, context, info));
}
