...
const NextQLError = _require.NextQLError;
...
class Model {
	...
	call(methodName, params = {}, context = {}, info) {
		if (!this.methods[methodName]) {
			return Promise.reject(new NextQLError(`No method: ${this.name}.${methodName}`, info));
		}

		try {
			return Promise.resolve(this.methods[methodName].apply(this, [params, context]));
		} catch (e) {
	...
	get(value, fieldName, params = {}, context = {}, info) {
		try {
			if (this.computed[fieldName]) {
				return Promise.resolve(this.computed[fieldName].apply(this, [value, params, context]));
			}
		} catch (e) {
			return Promise.reject(e);
		}
		...
		if (this.fields[fieldName]) {
			...
			return Promise.reject(new NextQLError(`No field: ${this.name}.${fieldName}`, info));
		}
	}
}
